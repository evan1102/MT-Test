...
function loadFormOptions() {
    MobileCRM.UI.EntityForm.requestObject(function (entityForm) {
        ...
        hasTimelog = !entityForm.entity.isNew &&
        ...
        isEditable = !isProcessed || (isProcessed && isRejected && setupOptions.UseManagerApproval);
...
function loadFormItems(entityForm) {
    ...
    formItems.cost.upDownVisible = false;   // formItems.cost.isEnabled
...
function createEntryTypeFormItem() {
    ...
    MobileCRM.UI.EntityForm.requestObject(function (entityForm) {
        ...
        var isTechActivity = selected.appointment && selected[entityName].transactiontype === "UNBILLED";

        if (entryTypeItem) {
            entryTypeItem.value = selected[entityName].costtype;
            entryTypeItem.isEnabled = entityForm.entity.isNew && !isTechActivity;
...
function createAppointmentTextFormItem() {
    ...
    MobileCRM.UI.EntityForm.requestObject(function (entityForm) {
        ...
        var apptText = entityForm.entity.properties.gpjobnumber ?
            entityForm.entity.properties.gpjobnumber + ":" +
            entityForm.entity.properties.gpappointmentid : selected.appointment.primaryName;
...
function setDefaultBilledPayCode() {
    if (selected[entityName].paycodeid)
...
function formValueChanged(entityForm) {
    ...
    switch (entityForm.context.changedItem) {
        ...
        case "transactiondate":
            validateTime(detailView.getItemByName(entityForm.context.changedItem));
            detailView.getItemByName(entityForm.context.changedItem).errorMessage = null;
            break;
        case "hoursunits":
            if (hasTimelog) {
...
function workCrewSelected(workCrew) {
    ...
    MobileCRM.UI.EntityForm.requestObject(function (entityForm) {
        MobileCRM.DynamicEntity.loadById('paycode', entityForm.entity.properties.paycodeid.id, function (paycode) {
            MobileCRM.UI.IFrameForm.showModal("Work Crew Employees",
                "file://entity/workcrewemployee/workcrewemployee-list.html",
                options = {
...
function btnDeleteClicked(entityForm) {
    ...
    var confirmPopup = new MobileCRM.UI.MessageBox(
        "You have chosen to delete a timesheet entry that has not yet been synced.  Do you want to continue?");
        ...
        if (button === "Yes") {
            if (entityForm.entity.isNew) {
                ...
                MobileCRM.DynamicEntity.deleteById("laborexpense", selected[entityName].id, function () {
                    if (hasTimelog)
                        updateCorrespondingTimelog().then(function () {
                            if (entityForm.entity.properties.transactiontype === "UNBILLED")
                                checkResetApptStatus();
                            else
                                setCleanAndClose();
...
function updateCorrespondingTimelog() {
    var deferred = $.Deferred();

    fetchCorrespondingTimelogID().then(function (timelogID) {
        if (timelogID)
            MobileCRM.DynamicEntity.loadById("timelog", timelogID, function (timelog) {
                timelog.properties.laborexpenseid = null;
