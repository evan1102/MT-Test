...
const Validation = {
    ...
    Inspection: 'Complete Inspections',
    PreviewInvoice: 'Preview Invoice',
    PurchaseOrderDetail: 'Create a Purchase Order',
    RefrigerantTracking: 'Add Refrigerant Tracking',
...
//============== LOAD OPTIONS ================
function checkMaximizeForm() {
    MobileCRM.Platform.getDeviceInfo(function (info) {
        if (info.oSVersion.indexOf("Microsoft Windows NT 6.2.92") > -1) {
            MobileCRM.UI.Form.requestObject(function (form) {
                MobileCRM.Platform.requestObject(function (platform) {
                    if (platform.isMultiPanel && !form.isMaximized) {
                        MobileCRM.bridge.onGlobalEvent("CalendarIsOpen", maximizeForm, true);
                        MobileCRM.bridge.raiseGlobalEvent("CheckForCalendar");
                    }
    ...
    MobileCRM.UI.EntityForm.requestObject(function (entityForm) {
        switch (item) {
            ...
            case Validation.Inventory:
                ...
                break;
            case Validation.Inspection:
                entityForm.selectTab("Inspections");
                break;
            ...
            case Validation.PurchaseOrderDetail:
                ...
                break;
            case Validation.RefrigerantTracking:
                if (selected.servicecall) {
                    fetchAssignedRefrigerantEquipment().then(function (refEquipment) {
                        if (refEquipment.length > 1) {// Multiple equipment assigned to call  
                            entityForm.selectTab(SCHEMA.equipment.name);
                        }
                        else {// Only one equipment assigned to call                          
                            navigateToTab(SCHEMA.equipment.name, refEquipment[0]['equipment.id'], SCHEMA.refrigeranttracking.name);
                        }
                    }, alertError);
                }
                break;
...
function fetchLaborExpense(costType) {
    ...
    entity.addFilter().where(SCHEMA.laborexpense.Properties.costtype, 'eq', costType);
    entity.addFilter().where(SCHEMA.laborexpense.Properties.appointmentid, 'eq', selected[entityName].id);
    entity.filter.type = 'and';

    // Only get laborexpense for current user, no work crew entries
    var employeeLink = entity.addLink(
        SCHEMA.employee.name,
        SCHEMA.employee.Properties.id,
        SCHEMA.laborexpense.Properties.employeeid,
        'inner');
    var systemuserLink = employeeLink.addLink(
        SCHEMA.systemuser.name,
        SCHEMA.systemuser.Properties.gpemployeeid,
...
// ----- Form Validation: PO Lines -----
function fetchPurchaseOrderDetail() {
    var deferred = $.Deferred();
    if (!selected.appointment) {
        return deferred.reject("Fetch PO Line Error: Missing appointment details");
    }
...
function checkSignatures() {
    ...
    if (!setupOptions.UseCustomerSignature && !setupOptions.UseTechnicianSignature)
        return deferred.resolve({});
    if (parseInt(selected.appointment.gpappointmenttype) === 1 && !setupOptions.GenerateServiceSummaryReport) {
        return deferred.resolve({});
    }
    if (parseInt(selected.appointment.gpappointmenttype) === 3 && !setupOptions.GenerateJobSummaryReport) {
        return deferred.resolve({});
    }

    fetchSummaryReport().then(function (summaryReport) {
        if (!summaryReport) {
            return deferred.reject("Check Report Signatures Error: Missing Summary Report");
        }
        var validationItems = {};
...
function formValueChanged(entityForm) {
    ...
    else if (entityForm.context.changedItem === "^BtnAttachment") {
        addAttachment();
    }
...
function confirmCompletion() {
    var msg = MobileCRM.Localization.get("Alert.ConfirmCompletion");
    var popup = new MobileCRM.UI.MessageBox(msg);
    MobileCRM.Metadata.requestObject(function (metadata) {
        // Check can create appointments
        if (metadata.entities.appointment.canCreate()) {
            popup.items = [
                MobileCRM.Localization.get("Cmd.CompleteApp"),
                MobileCRM.Localization.get("Cmd.CompleteCreateNewApp"),
        ...
        popup.show(function (btn) {
            if (btn === MobileCRM.Localization.get("Cmd.CompleteApp"))
                completeAppointment(false);
            if (btn === MobileCRM.Localization.get("Cmd.CompleteCreateNewApp"))
...
function requestReportPreviews(appointment) {
    ...
    fetchAppointmentReports(appointment.id).then(function (reports) {
        try {
            ...
            return deferred.reject("Request Report Previews " + e);
        }
    }, function (err) { return deferred.reject(err); });
    return deferred.promise();
}
function fetchAppointmentReports(appointmentID) {
    ...
    try {
        ...
        return deferred.reject("Fetch Appointment Reports " + e);
    }
    return deferred.promise();
}
...
function updateLocationContacts() {
    ...
    try {
        ...
        return deferred.reject("Update Location Contacts " + e);
    }
    return deferred.promise();
}
function deleteSignatureLocationContacts() {
    ...
    fetchSignatureLocationContacts().then(function (contactsToDelete) {
        try {
            ...
            return deferred.reject("Delete Signature Location Contacts " + e);
        }
    }, function (err) { return deferred.reject(err); });
    return deferred.promise();
}
function fetchSignatureLocationContacts() {
    ...
    try {
        ...
        return deferred.reject("Fetch Signature Location Contacts " + e);
    }
    return deferred.promise();
}
...
function deleteResolutionAnnotation() {
    var deferred = $.Deferred();
    if (!selected.appointment) {
        return deferred.reject("Delete Resolution Annotation Error: Missing Appointment Details");
    }
    try {
        var entity = new MobileCRM.FetchXml.Entity(SCHEMA.annotation.name);
        entity.addAttribute(SCHEMA.annotation.Properties.id);

        entity.addFilter().where(SCHEMA.annotation.Properties.gpnotetype, 'eq', "A");
        entity.addFilter().where(SCHEMA.annotation.Properties.subject, 'eq', "Resolution");
        entity.addFilter().where(SCHEMA.annotation.Properties.objectid, 'eq', selected.appointment.id);
        ...
        fetch.execute("JSON", function (res) {
            if (res.length === 0) {
                ...
                MobileCRM.DynamicEntity.deleteById(SCHEMA.annotation.name, res[0].id, function () {
                    return deferred.resolve();
                }, function (err) {
                    return deferred.reject("Delete Resolution Annotation Error: " + err);
                });
            }
        }, function (err) {
            return deferred.reject("Fetch Resolution Annotation Error: " + err);
        });
    }
    catch (e) {
        return deferred.reject("Delete Resolution Annotation " + e);
    }
    return deferred.promise();
}