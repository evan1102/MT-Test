...
function repaintScrollView(scrollView) {
    try {
        MobileCRM.bridge.getWindowSize(function (obj) {
            try {
                scrollView.option("height", obj.height - scrollHeight);
            }
            catch (e) {
                scrollView.option("height", obj.height - 110);
            }
        }, MobileCRM.bridge.alert);
    }
    catch (e) { }
...
function checkCanCreate(entityName) {
    var deferred = $.Deferred();
    MobileCRM.Metadata.requestObject(function (metadata) {
        return deferred.resolve(metadata.entities[entityName].canCreate());
    }, function (err) {
        return deferred.reject("Request Metadata for " + entityName + " error: " + err);
    }, null);
    return deferred.promise();
}
...
function checkConnection() {
    ...
    var networkCheck = new MobileCRM.FetchXml.Entity("setupoption");
    networkCheck.addFilter().where('name', 'eq', 'UseServerMode');
    var networkFetch = new MobileCRM.FetchXml.Fetch(networkCheck);
    networkFetch.execute("Online.JSON", function () {
...
function deleteCorrespondingAttachments(objectID) {
    var deferred = $.Deferred();
    fetchCorrespondingAttachments(objectID).then(function (attachments) {
        var itemsDeferred = [];

        $(attachments).each(function (_, attachment) {
            itemsDeferred.push(deleteAttachment(attachment));
        });
...
function fetchCorrespondingAttachments(objectID) {
    var deferred = $.Deferred();
    ...
    try {
        ...
        entity.addFilter().where(SCHEMA.annotation.Properties.objectid, 'eq', objectID);
        entity.addFilter().where(SCHEMA.annotation.Properties.isdocument, 'eq', 1);

        var fetch = new MobileCRM.FetchXml.Fetch(entity);
        fetch.execute("JSON", function (res) {
            return deferred.resolve(res);
        }, function (err) {
            return deferred.reject("Fetch Corresponding Attachments Error: " + err);
        });
    }
    catch (e) {
        return deferred.reject("Fetch Corresponding Attachments Error: " + e);
    }
    return deferred.promise();
}
function deleteAttachment(attachment) {
    var deferred = $.Deferred();
    if (!attachment) {
        return deferred.reject("Delete Attachment Error: Missing Attachment Details");
    }

    try {
        MobileCRM.DynamicEntity.deleteById(SCHEMA.annotation.name, attachment.id, function () {
            return deferred.resolve();
        }, function (err) {
            return deferred.reject("Delete Attachment Error: " + err);
        });
    }
    catch (e) {
        return deferred.reject("Delete Attachment Error: " + e);
    }
    return deferred.promise();
}
...
// ============== TIMESTAMP FUNCTIONS ================
function updateAppointmentStatusTimeStamp(appt, prevStatusName, newStatusName, isTimelog) {
    // appt needs id, gpservicecallid, gpappointmentid
    // prevStatusName & newStatusName to be text with status's name
    var deferred = $.Deferred();
    // returns new timestamp if one doesn't exist yet
    fetchAppointmentStatusTimeStamp(appt.id).then(function (timestamp) {
        if (timestamp.name !== newStatusName) {
            var newTimestamp = new MobileCRM.DynamicEntity(SCHEMA.appointmentstatustimestamp.name);
            updateTimeStamp(newTimestamp, appt, newStatusName, isTimelog).then(function () {
                return deferred.resolve(true);
            }, function (err) { return deferred.reject(err); });
        }
    }, function (err) { return deferred.reject(err); });
...
function fetchAppointmentStatusTimeStamp(apptId) {
    var deferred = $.Deferred();
    if (!apptId) {
        return deferred.reject("Fetch Appointment Status Time Stamp Error: Missing appointment details.");
    }

    var entity = new MobileCRM.FetchXml.Entity(SCHEMA.appointmentstatustimestamp.name);
    entity.orderBy(SCHEMA.appointmentstatustimestamp.Properties.modifiedon, true);
    entity.addAttributes();
    entity.addFilter().where(SCHEMA.appointmentstatustimestamp.Properties.appointmentid, 'eq', apptId);

    var fetch = new MobileCRM.FetchXml.Fetch(entity);
    fetch.execute("DynamicEntities", function (res) {
        var timestamp = res[0] ? res[0] : new MobileCRM.DynamicEntity(SCHEMA.appointmentstatustimestamp.name);
        return deferred.resolve(timestamp);
    }, function (err) {
        return deferred.reject("Fetch Appointment Status Time Stamp Error: " + err);
    });
    return deferred.promise();
}
function updateTimeStamp(timestamp, appt, statusName, isTimelog) {
    var deferred = $.Deferred();
    try {
        timestamp.properties.name = statusName;
        timestamp.properties.appointmentstatus = statusName;
        timestamp.properties.appointmentstatusdate = new Date();

        timestamp.properties.gpappointmentid = appt.gpappointmentid;
        timestamp.properties.gpservicecallid = appt.gpservicecallid;
        timestamp.properties.appointmentid = new MobileCRM.DynamicEntity(SCHEMA.appointment.name, appt.id);

        timestamp.save(function (err) {
            if (err) {
                return deferred.reject("Save Time Stamp Error: " + err);
            }
            else {
                if (isTimelog) {    // Update mobileaudit.resco_description = statusName
                    MobileCRM.bridge.raiseGlobalEvent("TimestampUpdated", { timestampId: this.id, statusName: statusName });
                }
                return deferred.resolve();
            }
        });
        ...
        return deferred.reject("Update Time Stamp Error: " + e);
    }
    return deferred.promise();
}

// ============== INSPECTION FUNCTIONS ================
function setInspectionReportsTabVisibility() {
    MobileCRM.Metadata.requestObject(function (metadata) {
        var inspectionEntity = MobileCRM.Metadata.getEntity(SCHEMA.resco_questionnaire.name);
        if (inspectionEntity === undefined) {
            MobileCRM.UI.EntityForm.requestObject(function (entityForm) {
                entityForm.setTabVisibility("report", false);
            }, alertError);
        }
    }, alertError);
}